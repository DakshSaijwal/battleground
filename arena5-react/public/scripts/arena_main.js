...
(function()
{
   ...
   extend(Arena.BaseGameScene, Game.Scene,
   {
      ...
      onInitScene: function onInitScene()
      {
         ...
         this.world.viewx = (this.world.size / 2) - (this.world.viewwidth / 2);
         this.world.viewy = (this.world.size / 2) - (this.world.viewheight / 2);
      },
...
(function()
{
   ...
   extend(Arena.GameScene, Arena.BaseGameScene,
   {
      ...
      onBeforeRenderScene: function onBeforeRenderScene()
      {
         ...
         var viewedgeX = w.viewwidth * 0.2;
         var viewedgeY = w.viewheight * 0.2;
         // adjust view to keep player in visible area
         if (p.position.x > viewedgeX && p.position.x < w.size - viewedgeX)
         {
            w.viewx = p.position.x - w.viewwidth * 0.5;
         }
         
         if (p.position.y > viewedgeY && p.position.y < w.size - viewedgeY)
         {
            w.viewy = p.position.y - w.viewheight * 0.5;
         }
      ...
      renderBackground: function renderBackground(ctx)
      {
         ...
         var w = this.world;
             ...
             x2 = (w.viewx < w.size - w.viewwidth ? w.viewwidth : w.size - w.viewx) * w.scale,
             y2 = (w.viewy < w.size - w.viewheight ? w.viewheight : w.size - w.viewy) * w.scale;
         
         // plot the grid wires that make up the background
         var loops = Math.max(w.viewwidth, w.viewheight) / UNIT;
         for (var i=0, j=loops; i<j; i++)
         {
         ...
         else if (w.viewx >= w.size - w.viewwidth)
         {
            xoff = w.size - w.viewx;
            ctx.moveTo(xoff * w.scale, y1);
            ctx.lineTo(xoff * w.scale, y2);
         ...
         else if (w.viewy >= w.size - w.viewheight)
         {
            yoff = w.size - w.viewy;
            ctx.moveTo(x1, yoff * w.scale);
            ctx.lineTo(x2, yoff * w.scale);
