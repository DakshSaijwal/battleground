...
Game.worldToScreen = function worldToScreen(vector, world, radiusx, radiusy)
{
   ...
   if (viewx < world.viewwidth + radiusx && viewy < world.viewheight + radiusy &&
...
(function()
{
   ...
   Game.Main.prototype =
   {
      ...
      frame: function frame()
      {
         ...
         if (this.currentScene && this.currentScene.world)
         {
            var minSize = Math.min(GameHandler.width, GameHandler.height);
            this.currentScene.world.scale = minSize / this.currentScene.world.viewsize;
            this.currentScene.world.viewwidth = GameHandler.width / this.currentScene.world.scale;
            this.currentScene.world.viewheight = GameHandler.height / this.currentScene.world.scale;
         }
         ...
         var updateSceneView = function(scene) {
            if (scene && scene.world) {
               var minSize = Math.min(GameHandler.width, GameHandler.height);
               scene.world.scale = minSize / scene.world.viewsize;
               scene.world.viewwidth = GameHandler.width / scene.world.scale;
               scene.world.viewheight = GameHandler.height / scene.world.scale;
            }
         };
         ...
         if ((!currentScene.interval || currentScene.interval.complete) && currentScene.isComplete())
         {
            ...
            if (!currentScene.inited)
            {
               currentScene.inited = true;
               currentScene.onInitScene();
            }
         }
